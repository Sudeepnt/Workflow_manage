...
// These are the seven Google-verified locator records being restored after the
// manual audit. The store numbers are kept stable so existing map searches and
// operational references continue to point at the same stores.
...
function getCatalogStore(storeNumber) {
  const name = RESTORE_TARGETS.get(storeNumber);
  return catalogData.stores.find((store) => store.name === name) || null;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const rows = storeNumbers.map((storeNumber) => {
      const store = getCatalogStore(storeNumber);
    ...
    const { data: existing, error: existingError } = await supabase
      ...
      .select("id, store_number, google_place_id")
